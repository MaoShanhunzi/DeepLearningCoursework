...
def model(train_img, learning_rate=0.001,num_epoch=50, batch_size=32, is_plot=True):
    ...
    network = FullyConnectedNet(180*180*3,10).to(device)
    ...
    if is_plot:
        ...
        plt.ylabel('Fullyconnected cost')
        plt.show()

    torch.save(network.state_dict(),'FullyConnectedNet_para.pkl')
    print('para saved')

def accuracy(dataset,batchsize):
    network = FullyConnectedNet.to(device)
